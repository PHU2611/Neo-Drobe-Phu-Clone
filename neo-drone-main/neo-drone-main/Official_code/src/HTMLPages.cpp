...
const char HTML_CONTROL_PAGE[] = R"rawliteral(
<!DOCTYPE html>
<html>
<head>
...
<body>
  ...
  <script>
    ...
    joy.addEventListener('touchmove', e => {
      ...
      let x = touch.clientX - rect.left -25; // center of stick
      let y = touch.clientY - rect.top -25; // center of stick
      ...
      rollX = Math.round((dx / 50) * 30);
      rollY = Math.round((-dy / 50) * 30);
      rollX = Math.max(-30, Math.min(30, rollX));
      rollY = Math.max(-30, Math.min(30, rollY));
    });
